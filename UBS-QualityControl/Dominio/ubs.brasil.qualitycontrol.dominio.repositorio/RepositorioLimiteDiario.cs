...
{
    public class RepositorioLimiteDiario : Repositorio<LimitePerfilRisco>
    {        
        private readonly RepositorioModeloQC dbQC;
        private readonly RepositorioModeloWM_DB dbWM_DB;

        public RepositorioLimiteDiario(RepositorioModeloQC repositorioModeloQC, RepositorioModeloWM_DB repositorioModeloWM_DB)
        {
        ...
        public override List<LimitePerfilRisco> SelecionarRegistro(LimitePerfilRisco limitePerfilRisco)
        {
            ...
            if (limitePerfilRisco.dtInicial != null)
                resQC = resQC.Where(item => item.dtIniVigencia >= limitePerfilRisco.dtInicial);

            if (limitePerfilRisco.dtFinal != null)
                resQC = resQC.Where(item => item.dtFimVigencia <= limitePerfilRisco.dtFinal);
            ...
            foreach (LimitePerfilRisco limitePerfil in limitePerfilRisco)
            {
                var resQC = from lpr in dbQC.LimitePerfilRisco
                            where
                              ...
                              ((limitePerfil.dtIniVigencia >= lpr.dtIniVigencia && limitePerfil.dtIniVigencia <= lpr.dtFimVigencia) ||
                               (limitePerfil.dtFimVigencia >= lpr.dtIniVigencia && limitePerfil.dtFimVigencia <= lpr.dtFimVigencia) ||                              
                              (limitePerfil.dtIniVigencia >= lpr.dtIniVigencia && limitePerfil.dtIniVigencia <= lpr.dtFimVigencia) ||
                               (limitePerfil.dtIniVigencia <= lpr.dtIniVigencia && limitePerfil.dtFimVigencia >= lpr.dtFimVigencia)) &&
