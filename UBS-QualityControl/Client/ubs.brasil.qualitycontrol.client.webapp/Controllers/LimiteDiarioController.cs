...
{
    [Authorize]  
    public class LimiteDiarioController : Controller
    {
        ...
        public LimiteDiarioController()
        {            
            this.repositorioModeloQC = new RepositorioModeloQC();
            this.repositorioModeloWM_DB = new RepositorioModeloWM_DB();

            this.fabricaOperacao = OperacaoFabrica.CriarOperacao<LimitePerfilRisco, LimiteDiario>(repositorioModeloQC, repositorioModeloWM_DB);
            ...
            this.fabricaRepositorioLimite = RepositorioFabrica.CriarRepositorio<LimitePerfilRisco, RepositorioLimiteDiario>(repositorioModeloQC, repositorioModeloWM_DB);   
        ...
        public ActionResult LimiteDiario(LimitePerfilRisco model)
        {                         
            return View(model);
        }
        ...
        [HttpPost]
        [ValidateInput(false)]
        public ActionResult LimiteDiario(LimitePerfilRisco model, FormCollection form)
        {       
            return View(model);
        } 

        #endregion LimiteDiario

        #region LimiteDiarioOperacao
        
        public ActionResult LimiteDiarioOperacao()
        {            
            return View();
        }
        ...
        [ChildActionOnly]
        public ActionResult LimiteDiarioOperacaoList(string codLimitePerfilRisco, string dtIniVigencia, string dtFimVigencia)
        {
            LimitePerfilRisco limitePerfilRisco = new LimitePerfilRisco();
            ...
            var lstRet = fabricaRepositorioLimite.SelecionarRegistro(limitePerfilRisco);
            if (lstRet.Count > 0) lstRet[0].Acao = "diario";

            return PartialView("../Operacao/_LimiteRiscoOperacaoPartial", lstRet);              
        }

        [HttpPost]            
        public ActionResult LimiteDiarioOperacao(LimitePerfilRisco model, FormCollection form)
        {
            ...
            try
            {
                switch (acao.ToUpper())
                {
                    case "INCLUIR":
                    {
                        int retInclusao = IncluirLimiteDiario(form);

                        if (retInclusao > 0)
                            model.Msg = "* Limite Diário incluído.";
                        else                                              
                            if (retInclusao == -1)
                                model.Msg = "* Já existe limite para esta data. Inclusão não efetuada.";
                            else
                                model.Msg = "* Erro: Limite Diário não incluído.";                        
                        break;
                    }
                    case "ALTERAR":
                        if (AlterarLimiteDiario(form) > 0)
                            model.Msg = "* Limite Diário alterado.";
                        else
                            model.Msg = "* Erro: Limite Diário não alterado.";
                        break;
                    case "EXCLUIR":                        
                        if (ExcluirLimiteDiario(form) > 0)
                            model.Msg = "* Limite Diário excluído.";
                        else
                            model.Msg = "* Erro: Limite Diário não encontrado para exclusão.";
                        break;
                    default:
        ...
        private int IncluirLimiteDiario(FormCollection form)
        {
            return fabricaOperacao.Gravar(MontaParamsIncluir(form));
        }

        private int AlterarLimiteDiario(FormCollection form)
        {
            return fabricaOperacao.Alterar(MontaParamsAlterarExcluir(form));
        }

        private int ExcluirLimiteDiario(FormCollection form)
        {
        ...
        private List<LimitePerfilRisco> MontaParamsIncluir(FormCollection form)
        {
            ...
            for (int i = 0; i < countform; i++)
            {
                ...
                perfilRiscoPatrimonial.dtFimVigencia = DateTime.ParseExact(form["dtFimVigencia"], "dd/MM/yyyy", cultureData); 
                ...
                perfilRiscoCota.dtFimVigencia = DateTime.ParseExact(form["dtFimVigencia"], "dd/MM/yyyy", cultureData);
        ...
        #endregion LimiteDiarioOperacao
    }
}
